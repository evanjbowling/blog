...
# Builds slides.html from presentation.md using pandoc + reveal.js.
#
# --slide-level=2
...
#   to be served over HTTP — opening slides.html directly as a local file
...
pandoc presentation.md -t revealjs -s -o slides.html --css style.css --include-after-body footer.html -V theme=white --syntax-highlighting=tango --slide-level=2
sed -i '' 's/data-src=/src=/g' slides.html
